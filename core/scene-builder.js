...
export const FURNITURE_GEOMETRY_RULES = Object.freeze({
  sofa: 'The sofa is a single discrete upholstered piece with one unified frame, seat cushions, back cushions, and two armrests. All legs/base contact the ground. When a subject is seated, the pelvis visibly compresses one seat cushion, the thighs align with the seat plane, the back meets a back cushion where the pose requires support, and both feet rest on the floor. Preserve smooth matte upholstery without pattern, print, or contrasting fabric. Keep its silhouette and contact boundaries independent from every background sofa.',
  armchair: 'The armchair is a single discrete one-seat piece with one seat cushion and one back cushion leaning 10-15 degrees from vertical. It has two rounded padded armrests, each 10-14 cm wide, and four short wooden legs with all four feet in firm floor contact and coherent contact shadows. When a subject is seated, the pelvis visibly compresses the seat cushion, the thighs follow the seat plane, the back contacts the back cushion according to posture, and the feet rest naturally on the floor unless the pose explicitly requires another placement. It must remain visually separate from every sofa and background chair.',
  chair: 'The chair is a single discrete one-seat piece with one seat pad and one backrest or back cushion leaning 8-12 degrees from vertical. It has two narrow flat armrests, each 4-6 cm wide, and four legs with all four feet contacting the floor and casting coherent contact shadows. When a subject is seated, the pelvis creates visible seat compression where padding exists, the thighs align with the seat plane, the back contacts the backrest according to posture, and the feet rest naturally on the floor unless the pose explicitly says otherwise. It must not merge with a desk, sofa, wall, or background chair.',
...
export const LAPTOP_SCENE_CONTEXTS = Object.freeze({
  bedroom_seated: 'A slim closed or open modern MacBook rests on the bed surface, the lone nightstand, the dresser top, or the floor beside the subject for floor-sitting poses, at natural scale, switched off or dim. Never held unless an explicit laptop-on-lap bedroom pose requires it.',
...
export function getFurnitureGeometryForScene(sceneType, location = '', poseValue = '') {
  ...
  if(semantic==='counter' && !(COUNTER_CONTEXT.has(sceneType) || COUNTER_CONTEXT.has(location))) return [FURNITURE_GEOMETRY_RULES.generic];
...
['مجالس ومنازل','saudi_bedroom_livedin','غرفة نوم سعودية واقعية','inside one fixed lived-in Saudi bedroom with the bed and dark tufted headboard locked to the left wall, exactly one nightstand, one fixed compact bedroom chair near the front wall with a draped garment, sliding mirrored wardrobe doors and a separate dark-wood dresser on the right wall, fully closed black-to-charcoal curtains across the back wall, a high front-wall split air conditioner, fixed recessed ceiling spots, glossy light-beige tile flooring, one large central rug, a dark soft bag near the dresser, two shoe pairs near the rug edge, and stable ordinary signs of daily use',['front_selfie','standing_selfie','seated_selfie','mirror_selfie','third_person_portrait','full_body_third_person','candid_third_person','home_interior_casual_selfie','mirror_bedroom_selfie','reclining_bed_selfie','lying_bed_selfie','phone_screen_only_selfie']],
...
export const MAJLIS_ANCHOR = Object.freeze({
  ...
  main_sofa: 'A light-sand modern sofa runs along the RIGHT wall as one connected piece. All cushions are smooth matte light-sand upholstery with a uniform monochrome surface. No dots, no speckles, no checkered or grid pattern, no knitted texture, no contrasting threads, no tufting, no buttons, no quilting.',
  main_sofa_geometry: 'The main sofa is ONE connected L-shaped sectional, never a straight three-seat sofa and never multiple merged sofas. Its long three-seat run is parallel to and flush against the RIGHT wall with no visible floor gap. At the BACK end of this long run, one short connected return projects inward toward the room center at approximately 90 degrees. The long run retains three distinct seat cushions. The corner/return remains physically connected to the same sofa structure and must never become a separate chair or detached seat. Armrests remain flat horizontal surfaces and never become a block, seat, or additional cushion.',
  side_sofas: 'Two separate single-seat sofas in the same light-sand upholstery sit along the LEFT wall facing the L-shape. Each has one seat cushion, one back cushion, its own armrests and legs, and an independent silhouette. The backs of both single sofas are flush against the LEFT wall with no visible floor gap.',
...
export const BEDROOM_ANCHOR = Object.freeze({
  room: 'A single fixed master bedroom, roughly 4m x 5m, with cream-painted walls, crown molding, glossy light-beige tile flooring, and one stable furniture layout reused in every bedroom image. Camera position may change, but the room geometry does not.',
  coordinate_frame: 'Use one immutable room coordinate frame: LEFT wall = bed/headboard zone plus the entrance door toward the front; RIGHT wall = wardrobe and dresser zone; BACK wall = fully closed dark curtains; FRONT wall = high split air conditioner. Preserve wall-to-wall distances, corner order, object scale, and vanishing directions across every view.',
  door: 'Exactly one medium-dark wooden hinged door is fixed on the FRONT half of the LEFT wall. Its frame, hinge side, handle height, and swing clearance remain unchanged, and the door never intersects the bed or nightstand.',
  bed: 'A queen-size bed with an approximately 160 x 200 cm mattress and a dark tufted headboard is fixed against the LEFT wall, with the long axis extending into the room. Grey bedding stays on the same bed. Exactly one nightstand exists beside the headboard; the bed, headboard, mattress and nightstand never migrate, rotate, duplicate or change size.',
  bedroom_chair: 'Exactly one compact upholstered bedroom chair is fixed against the FRONT wall slightly right of center, angled only slightly toward the room center. It has one seat cushion, one backrest and grounded legs. A single casually draped garment may remain on its backrest or seat. Do not create a second chair.',
  wardrobe: 'A wide dark-wood wardrobe with sliding mirrored or reflective glass doors is fixed on the RIGHT wall, mainly in the back half of the room, at realistic wardrobe depth. Door tracks, panel seams, handles and reflective panels keep the same count and spacing in every image.',
  mirror: 'There is no separate framed mirror. Mirror reflections come only from the wardrobe sliding reflective panels on the RIGHT wall. Reflections obey the physical reflection cone, reverse lateral geometry consistently, preserve panel seams, and show only objects that are actually visible from the reflected viewpoint.',
  dresser: 'A dark-wood chest of drawers is fixed on the RIGHT wall toward the FRONT half, separate from the wardrobe with a visible walking gap. Drawer count, top height, depth and footprint remain stable; it never merges into the wardrobe.',
  nightstand: 'Exactly one dark-wood nightstand exists beside the bed near the headboard, with one small bedside lamp and ordinary personal items. The lamp may be on or off according to the selected lighting, but the nightstand and lamp never change position or duplicate.',
  curtains: 'Fully closed black-to-charcoal curtains span the BACK wall from a ceiling track to just above the floor. The fabric hangs under gravity with vertical folds and restrained lower pooling. No window frame, glazing, daylight gap or exterior view is visible through or beside the curtains.',
  air_conditioner: 'One wall-mounted split air conditioner is fixed high on the FRONT wall. Its indoor unit remains the same size and position, with physically plausible shadowing and no duplicate unit.',
  ceiling: 'Six recessed ceiling spotlights are arranged in a fixed two-by-three grid. Their fixture positions never move. Individual lights may be off according to the selected lighting profile; do not invent extra ceiling fixtures.',
  rug: 'One large rectangular low-pile rug is fixed across the open central floor zone, aligned with the room geometry and clear of wardrobe door travel. Its corners, orientation and approximate footprint remain stable, with only local pile compression where feet or objects contact it.',
  circulation: 'Preserve a believable clear walking path from the entrance to the bed, chair, dresser and wardrobe. Furniture may not intersect, block the door swing, overlap wardrobe sliding-door travel, or shrink the aisle to implausible dimensions.',
  fixed_daily_items: 'Keep a small set of lived-in anchors in stable zones: one dark soft bag near the dresser base, two pairs of shoes near the front-right edge of the rug, one draped garment on the fixed chair, and one charging cable associated with the lone nightstand. Clutter level may add secondary small items, but these anchors must not teleport, duplicate or switch sides.',
  materials: 'Use restrained residential materials: dark wood with visible but subtle grain on wardrobe, dresser and nightstand; a dark tufted upholstered headboard; grey cotton bedding; low-pile woven rug; glossy light-beige ceramic tile with continuous grout lines; matte cream paint; dark curtain fabric. Surfaces show ordinary wear and micro-imperfections, never showroom polish.',
  fixed_layout_rule: 'IMPORTANT: This is one locked bedroom, not a bedroom category. Keep the same door, bed, headboard, single nightstand, fixed bedroom chair, wardrobe, reflective panels, dresser, curtains, air conditioner, ceiling-light grid, rug, material palette and fixed daily-item zones in every bedroom image. Do not move, add, remove, resize, rotate, mirror, duplicate or substitute these elements. Do not add a second nightstand, second chair, sofa, desk, television, freestanding mirror or visible window. Camera framing may hide an element but must never relocate or replace it.'
});

export const BEDROOM_CLUTTER_LEVELS = Object.freeze({
  clean: 'Grey bedding is neatly made with mild natural fabric tension; only the fixed bag, shoe pairs, draped chair garment and nightstand charging cable remain visible, with no additional loose clutter.',
  minimal: 'Grey bedding is slightly rumpled; add one mug on the lone nightstand and one folded item on the dresser while keeping the fixed daily-item anchors in their locked zones.',
  light: 'Grey bedding has ordinary sleep wrinkles; add one mug and headphones on the lone nightstand, one open book on the dresser, and one small folded clothing item without changing any fixed furniture or anchor-item position.',
  moderate: 'Grey bedding is naturally rumpled with two displaced pillows; the lone nightstand holds one mug, headphones and its charging cable; the dresser holds one open book and a small folded clothing stack; the fixed bag, two shoe pairs and draped chair garment remain in their locked zones. Keep the mess sparse enough to preserve clear circulation.',
  heavy: 'Grey bedding is heavily rumpled with a displaced blanket and pillows; add a few clothing pieces on the bed edge and dresser, one extra cup or small plate, and a small cluster of cables near the nightstand. Keep the fixed bag, shoe pairs and chair garment in place, preserve walking clearance, and never use clutter to create extra furniture.'
});

export const BEDROOM_REALISM_RULES = Object.freeze({
  scale: 'ROOM SCALE: Keep human-to-room scale stable: the approximately 160 x 200 cm bed, standard door height, wardrobe depth, dresser height, chair size and walking clearances must agree with the subject body and camera perspective in the same metric world.',
  ...
  lighting: 'LIGHTING CAUSALITY: Every highlight, cast shadow, reflection and exposure gradient must be explainable by the selected practical sources: recessed ceiling spots, bedside lamp, phone screen, or daylight only when the lighting profile explicitly allows it. No studio fill, rim light, floating glow or HDR shadow lifting.',
  occlusion: 'OCCLUSION / DEPTH: Preserve one consistent depth order between subject, bed, chair, rug, nightstand, dresser, wardrobe and curtains. Near objects must occlude far objects correctly; no furniture can appear both in front of and behind the same body part.',
  continuity: 'CONTINUITY LOCK: Different poses and camera angles are alternate photographs of the same physical room. Only viewpoint, pose, lighting state and allowed clutter level may change; fixed furniture geometry, material identity, wall assignment and anchor-item zones remain invariant.'
});

export function getBedroomRealismRules(poseValue = '', captureType = '') {
  const pose = String(poseValue || '').toLowerCase();
  const capture = String(captureType || '').toLowerCase();
  const rules = [
    BEDROOM_REALISM_RULES.scale,
    BEDROOM_REALISM_RULES.support_contact,
    BEDROOM_REALISM_RULES.reflection,
    BEDROOM_REALISM_RULES.floor_rug,
    BEDROOM_REALISM_RULES.lighting,
...
{ group:'جلوس على كرسي الغرفة', value:'armchair-sit-corner', label:'جلسة مائلة على كرسي غرفة النوم', prompt:'sitting slightly diagonal on the fixed bedroom chair with a mild torso rotation, without shifting the chair from its front-wall position; weight remains centered over the seat cushion', cameraHint:'front camera at eye level from a mild side angle; chair stays fixed' },
{ group:'جلوس على كرسي الغرفة', value:'armchair-sit-one-knee', label:'على كرسي الغرفة بركبة مرفوعة', prompt:'sitting on the fixed bedroom chair with one knee comfortably raised while the other foot stays grounded; pelvis and seat compression remain plausible and the chair does not move', cameraHint:'front camera slightly above eye level; fixed chair visible in its anchored position' },
{ group:'جلوس على كرسي الغرفة', value:'armchair-sit-crossed', label:'جالس مربع على كرسي غرفة النوم', prompt:'sitting cross-legged only if the fixed chair seat width physically allows it, with pelvis fully supported and shoulders relaxed; otherwise keep the legs loosely folded without forcing anatomy', cameraHint:'front camera at eye level, framed waist-up; chair remains fixed against the front wall' },
{ group:'جلوس على كرسي الغرفة', value:'armchair-sit-feet-floor', label:'جالس على كرسي الغرفة والقدمان على الأرض', prompt:'sitting naturally on the fixed bedroom chair with both feet on the rug or tile according to the anchored chair position, pelvis supported and forearms relaxed on the thighs', cameraHint:'front camera at eye level, centered framing; chair remains fixed' },
...
{ group:'وقوف', value:'bedroom-stand-window', label:'واقف عند الستائر الخلفية', prompt:'standing near the fully closed back-wall curtains with one shoulder closer to the fabric; the concealed window remains completely invisible and the curtains stay closed', cameraHint:'front camera at eye level near the back-wall curtains, with lighting determined only by the selected profile' },
...
{ group:'ماسك لابتوب', value:'bedroom-laptop-armchair', label:'لابتوب على الحضن فوق كرسي الغرفة', prompt:'sitting on the fixed bedroom chair with an open laptop supported across both thighs at natural scale, one free hand available for light keyboard contact while the phone-bearing arm remains independent; chair stays in its anchored front-wall position', cameraHint:'front camera at eye level while seated, laptop visible below chest and chair geometry stable' },
...
{ group:'ماسك شيء', value:'bedroom-tea-armchair', label:'ماسك شاي على كرسي غرفة النوم', prompt:'sitting on the fixed bedroom chair while lightly holding a small tea glass in the free hand below chest level; the chair remains anchored and body weight compresses the seat locally', cameraHint:'front camera at eye level while seated on the fixed chair, tea glass below chin' },
...
{ group:'تفاعل مع الغرفة', value:'bedroom-curtain-touch', label:'واقف يلمس طرف الستارة', prompt:'standing at the fully closed back-wall curtains, lightly pinching or resting the free hand on the outer fabric edge so only a small local fold changes; do not open a gap or reveal the concealed window', cameraHint:'front camera at eye level beside the curtain edge; back wall remains fully covered' },
...
{ group:'مرآة', value:'bedroom-mirror-seated', label:'أمام مرآة الخزانة جالس على كرسي الغرفة', prompt:'seated on the fixed bedroom chair in its anchored front-wall position and turned naturally toward the RIGHT-wall wardrobe mirror, with pelvis supported and no chair relocation; phone visible only through the physically correct reflection path', cameraHint:'seated mirror selfie using the wardrobe reflective panel; reflected camera axis, chair position and room geometry must agree' },
];
...
['ليلي داخلي','night_majlis_warm','مجلس — سبوتات دافئة','The central ceiling fixture and restrained ceiling spotlights are the dominant physical light sources, producing warm downward illumination and natural wall/floor bounce. The single side lamp is ON but weaker and contributes only localized warm light near its physical position. No other source may illuminate the subject or room.'],
