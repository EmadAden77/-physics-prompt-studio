(function(M){
  ...
  M.compile=function(prompt,state){
    ...
    const seat={driver:'Physical FRONT-LEFT DRIVER SEAT in a Saudi LHD vehicle. The subject, driver cushion, steering column, instrument cluster and pedals share one axis; driver door is beside his anatomical LEFT shoulder and center console beside his RIGHT thigh. Do not force the steering wheel into frame; if its rim naturally enters, it is only a thin correctly aligned fragment, never a dominant foreground object.',passenger:'Physical FRONT-RIGHT PASSENGER SEAT; center console at anatomical LEFT and passenger door at anatomical RIGHT; steering column remains across the console on vehicle left.','rear-left':'Physical rear-left seat behind the driver, with correct front-seat occlusion.','rear-right':'Physical rear-right seat behind the passenger, with correct front-seat occlusion.'};
...
Xiaomi 15 Ultra front camera, approximately 21mm-equivalent, ${cameraGeometry}, ${prompt.photography.aspect_ratio}. ${String(prompt.photography.angle).replace(/[.\s]+$/,'')}. Keep the face primary and crop before the waist. No fisheye distortion, oversized nose, stretched jaw, portrait-mode cutout, DSLR bokeh, studio polish or oversized foreground steering wheel.
