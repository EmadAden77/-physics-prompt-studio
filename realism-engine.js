...
const REALISM_MODULES = {
  ...
  vehicleInterior:{label:'Vehicle Interior',when:c=>c.captureType!=='exterior',prompt:'VEHICLE INTERIOR MODULE: for a Saudi LHD vehicle, validate occupancy through cabin topology rather than viewer-left/viewer-right. Driver = physical front-left seat on the steering-column, instrument-cluster and pedal axis; driver door at anatomical left shoulder; center console at anatomical right thigh; passenger seat beyond it. Reject and rebuild any swapped seat or relocated steering column.'},
  lowLight:{label:'Low Light',when:c=>c.time==='night',prompt:'LOW-LIGHT MODULE: apply plausible sensor noise, shadow-detail loss, reduced micro-detail, restrained sharpening, limited dynamic range and supported motion softness. Never clean or relight deep shadows artificially.'},
