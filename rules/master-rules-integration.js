(function(g){
  ...
  promptDoctor=function(){const result=previousDoctor(),audit=M.evaluate(state());result.ruleAudit=audit.results;result.verified.push(`V5: فُحصت ${audit.active.length} قواعد نشطة داخليًا؛ لا تُضاف تقارير المحرك إلى البرومبت.`);audit.conflicts.forEach(c=>result.verified.push(`${c.id}: صُحح تلقائيًا لصالح ${c.winner}`));return result};
