...
function verification(scene,ratio,guidance,hair,identityReference=true){
  ...
  const hairIdentityCheck=identityReference
    ? 'hair density, curl pattern, top volume, and length match the reference'
    : 'hair density, curl pattern, top volume, and length remain stable relative to the chosen no-reference baseline';
  const hairCheck=hairDirection==='neutral'
    ? 'hair remains naturally arranged without an invented directional lock'
    : `the selected hair direction is unmistakably visible; no strands contradict the selected direction; stray hairs (if visible) respect the selected direction; ${hairIdentityCheck}${hairDirection==='forward' ? '; only the direction of the front section has changed' : ''}`;
