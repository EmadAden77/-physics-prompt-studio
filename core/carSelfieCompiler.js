...
import { compileInsideSections } from './carSelfieInsideCompiler.js';
import { compileOutsideSections } from './carSelfieOutsideCompiler.js';
import { compileNarrative, NEGATIVE_LIST } from './carSelfieNarrativeCompiler.js';

const prompt = (field, id) => commonOption(field, id)?.prompt || '';

const LEGACY_HARD_ACCEPTANCE = `HARD ACCEPTANCE CRITERIA (image FAILS if any is violated):
...
4. Denim and other fabrics must show seam bunching, gravity-driven wrinkles, matte fiber response, and slight value variation.
5. Camera tilt must be slightly off-perfect-level.
6. Corneal reflections must show source-consistent catchlights.
7. No brand text, logo, or readable signage may appear unless explicitly requested.
8. Composition must be slightly off-center.`;
...
function legacyCommonSections(state) {
...
export function compileCarSelfieDetailed(input = {}) {
  ...
  return compileNarrative(state, state.mode);
}

export function compileCarSelfieTechnicalSpec(input = {}) {
  const state = normalizeCarState(input);
  const status = carValidationStatus(state);
  if (status.blocked) return blockedMessage(status);
  const modeSections = state.mode === 'outside' ? compileOutsideSections(state) : compileInsideSections(state);
  return [
    'CAR SELFIE PHYSICS STUDIO — TECHNICAL SPEC (LEGACY)',
    '',
    `ACTIVE MODE: ${state.mode === 'outside' ? 'OUTSIDE BESIDE CAR' : 'INSIDE CAR'}.`,
    ...legacyCommonSections(state),
    ...modeSections,
    state.notes.trim() ? `USER NOTES: ${state.notes.trim()}` : '',
    '',
    LEGACY_HARD_ACCEPTANCE
  ].filter(Boolean).join('\n');
}
...
export function compileCarSelfieJson(input = {}) {
  ...
  return JSON.stringify({
    version: 'car-selfie-v9-narrative',
    ...
    validation,
    outputs: {
      narrative: compileCarSelfieDetailed(state),
      concise: compileCarSelfieConcise(state),
      negative: compileCarSelfieNegative(state),
      technicalLegacy: compileCarSelfieTechnicalSpec(state)
    }
  }, null, 2);
}
