...
export const FURNITURE_GEOMETRY_RULES = Object.freeze({
  sofa: 'The main sofa is ONE connected L-shaped sectional, never a straight three-seat sofa and never multiple merged sofas. Its long three-seat run is parallel to and flush against the RIGHT wall. At the BACK end of this long run, one short connected return projects inward toward the room center at approximately 90 degrees. The long run retains three distinct seat cushions. The corner/return remains physically connected to the same sofa structure and must never become a separate chair or detached seat. Preserve coherent armrest, base, leg, cushion, floor-contact and wall-contact geometry throughout the entire sectional. It has two flat padded armrests, each 14-18 cm wide, and short wooden legs with every leg visibly grounded by coherent contact shadows. Armrests remain flat horizontal surfaces and never become a block, seat, or additional cushion. Upholstery is a smooth matte continuous surface in a single solid color, with no dots, no speckles, no grid, no checkered weave, no knitted texture, no visible thread pattern, no tufting, no buttons, no quilting, and no contrast between cushion fabric and back fabric. Sofa upholstery is deliberately smooth and untextured; it must NOT share the slub, linen, or woven-appearance texture of any clothing worn by the subject. When the scene positions a sofa against a wall, its back remains flush with the wall and no floor is visible behind it. When a subject is seated, the pelvis visibly compresses one seat cushion, the thighs align with the seat plane without sinking through it, the back meets a back cushion where the pose requires support, and both feet rest naturally on the floor unless the pose explicitly says otherwise. Keep its silhouette and contact boundaries independent from every background sofa or chair.',
...
export const MAJLIS_ANCHOR = Object.freeze({
  ...
  main_sofa: 'A light-sand modern L-shape sofa runs along the RIGHT wall as one connected piece, with three separate seat cushions, four square back cushions, and short wooden legs. All cushions are smooth matte light-sand upholstery with a uniform monochrome surface and no visible weave texture at viewing distance. No dots, no speckles, no checkered or grid pattern, no knitted texture, no contrasting threads, no tufting, no buttons, no quilting. The back of the L-shape sofa is flush against the RIGHT wall with no visible floor gap between the sofa back and the wall.',
